...
function actualizarDatos(){
  ...
  for(var i = 0;i < numCargas ; i++){
    ...
    var q = calCargaUno(i);
     arrayCargas.push(new CargaP(centro1X+posicion*PixelMet,centroY,4,q,i))
  }
  for(var i = 1;i < numCargas ; i++){
    var posicion = calPosDos(i);
    var q = calCargaDos(i);
    arrayCargas2.push(new CargaP(centro2X-posicion*PixelMet,centroY,4,q,i))
  }
...
async function actualizarFrame(){
  ...
  for(var j=0;j<arrayMedidores.length;j++){
    arrayMedidores[j].actualizarValor(0);
  ...
  for (var i = 0;i<arrayCargas.length;i++){
    ...
    for(var j=0;j<arrayMedidores.length;j++){
      arrayMedidores[j].actualizarValor(i+1);
      arrayMedidores[j].show();
    }
    await sleep(200); 
  }
...
function CargaP(x, y , r , q,i){
  ...
  this.show = function(){
    ...
    c.textAlign = "center";
    c.font = "15px Arial";
    c.fillText((this.q*Math.pow(10,9)).toPrecision(3)+"nC", this.x, this.y-i*25);
  }
...
function medidor(ix,iy,id){
  ...
  this.actualizarValor = function(conta){
    this.ValorV = 0;
...
function RePrint(){
  ...
  for(var j=0;j<arrayMedidores.length;j++){
    arrayMedidores[j].actualizarValor(numCargas);
    arrayMedidores[j].show();
  }
}